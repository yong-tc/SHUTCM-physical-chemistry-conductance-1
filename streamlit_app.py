...
# ==================== 页面配置（必须放在最前面）====================
st.set_page_config(page_title="电导法测定弱电解质电离平衡常数", layout="wide")
...
if st.button("🚀 开始计算", type="primary"):
    ...
    st.session_state.params = {
        ...
        "correction_factor": correction_factor if use_correction else None
    }
...
if st.session_state.calculated:
    ...
    # 动态显示图表（使用 components.html 实时渲染）
    st.subheader("📈 图表分析")
    tab1, tab2, tab3, tab4 = st.tabs(["Λₘ - c", "α - c", "Kc 分布", "Ostwald 稀释定律验证"])
    idx = 0
    with tab1:
        if idx < len(fig_html_list) and fig_html_list[idx][0] == '摩尔电导率随浓度的变化':
            st.components.v1.html(fig_html_list[idx][1], height=500)
            idx += 1
        else:
            st.info("数据点不足，无法绘制此图")
    with tab2:
        if idx < len(fig_html_list) and fig_html_list[idx][0] == '电离度随浓度的变化':
            st.components.v1.html(fig_html_list[idx][1], height=500)
            idx += 1
        else:
            st.info("数据点不足，无法绘制此图")
    with tab3:
        if idx < len(fig_html_list) and fig_html_list[idx][0] == '各浓度下的 Kc 值':
            st.components.v1.html(fig_html_list[idx][1], height=500)
            idx += 1
        else:
            st.info("数据点不足，无法绘制此图")
    with tab4:
        if idx < len(fig_html_list) and fig_html_list[idx][0] == 'Ostwald 稀释定律验证':
            st.components.v1.html(fig_html_list[idx][1], height=500)
            if ostwald_eq:
    ...
    with col3:
        ...
        if st.button("🖨️ 生成 PDF 报告"):
            ...
            """)
            for title, html in fig_html_list:
                pdf_html_parts.append(f'<div class="chart"><h3>{title}</h3>{html}</div>')
            ...
            """)
            full_pdf_html = "".join(pdf_html_parts)
            # 使用 components.html 嵌入并自动打印
            st.components.v1.html(full_pdf_html, height=0, scrolling=False)
            st.success("报告已生成，请在弹出的打印对话框中选择「另存为 PDF」。如果未弹出，请检查浏览器是否拦截了弹窗。")
